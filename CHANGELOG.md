...
- **Airgap QR keyshare** — Settings → Security (biometric-gated) is a two-step export on the glass modal: CSPRNG **6-digit PIN** first (“Eyes on these two screens only”; grouped `XXX XXX` display), then **Show QR** hides the PIN and plays an animated `ur:bytes` QR of AES-encrypted keyshare ciphertext (`sha256(PIN)` key). Export uses larger UR fragments (**500B**) and a faster cadence (**125ms**) so a ~140KB share is ~280 frames instead of ~700 at the send-QR defaults. **Restart frames** rewinds the fountain encoder from frame 1 if the receiver missed the start. Welcome **Restore via Airgap QR** enters that PIN (`inputMode: pin`), then continuous-scans until the UR completes and uses the same import path as a `.share` file (`pendingRestore` indexing). File backup/restore still uses a user-chosen password.
...
- **Network Providers “Healthy” in dark mode** — per-host quality hint used `primary` (`#3A3A3A`), which vanished on cards; now matches the header health-dot rule (teal `secondary` in dark, `primary` in light). Slow stays orange; Unreachable stays muted.
...
- **Health / send**: `services/mempoolHealth.ts`, `MempoolClient.ts`, `sendBalanceRecheck.ts`, `broadcastErrorMessages.ts`, `feeUtils.ts`, `CacheIndicator.tsx`, `syncFillWindow.ts`, `SyncCoordinator.ts`, `walletRefreshSession.ts`, `rateLimitRetry.ts` (`mapPool`, `MULTI_PROVIDER_SYNC_CONCURRENCY`).
...
- **Camouflage**: `IconChangerModule.java`, `MainApplication.kt`, `services/camouflagePresets.ts`, `services/camouflagePin.ts`, `CONFIG_KEYS.CAMOUFLAGE_PIN_FAIL_COUNT` / `CAMOUFLAGE_PIN_LOCKED_UNTIL`, `components/CamouflagePinPad.tsx`, `screens/WalletSettings.tsx`, `screens/LoadingScreen.tsx`, `assets/camouflage/*.png`; aliases `.DefaultIconActivity` / `.AlternativeIconActivity` / `.NotesIconActivity` / `.WeatherIconActivity` / `.FilesIconActivity`.
...
- **Airgap QR**: `services/airgapPin.ts`, `services/secureRandom.ts`; `utils/urBytesQr.ts` (`bufferToUr` / `receiveUrBytesPartAsBase64`, `UR_AIRGAP_FRAGMENT_SIZE` **500** / `UR_AIRGAP_FRAME_INTERVAL_MS` **125**); `components/AnimatedUrQr.tsx` (`restartNonce`, `maxFragmentLen`, `frameIntervalMs`); `AirgapKeyshareExportModal.tsx` (PIN → QR steps, glass overlay); `KeyshareImportPasswordModal.tsx` (`inputMode: pin`); Welcome restore in `ShowcaseScreen.tsx`; Settings Security + `WALLET_SENSITIVE_ACTION_AUTH.airgapKeyshareExport`. Native `BbmtSecureRandom` / `BBMTLibNativeModule.secureRandom`.
