...
- **Airgap QR keyshare** — Settings → Security (biometric-gated) is a two-step export on the glass modal: CSPRNG **6-digit PIN** first (“Eyes on these two screens only”; grouped `XXX XXX` display), then **Show QR** hides the PIN and plays an animated `ur:bytes` QR of AES-encrypted keyshare ciphertext (`sha256(PIN)` key). **Restart frames** rewinds the fountain encoder from frame 1 if the receiver missed the start. Welcome **Restore via Airgap QR** enters that PIN (`inputMode: pin`), then continuous-scans until the UR completes and uses the same import path as a `.share` file (`pendingRestore` indexing). File backup/restore still uses a user-chosen password.
...
- **Airgap QR**: `services/airgapPin.ts`, `services/secureRandom.ts`; `utils/urBytesQr.ts` (`bufferToUr` / `receiveUrBytesPartAsBase64`); `components/AnimatedUrQr.tsx` (`restartNonce`); `AirgapKeyshareExportModal.tsx` (PIN → QR steps, glass overlay); `KeyshareImportPasswordModal.tsx` (`inputMode: pin`); Welcome restore in `ShowcaseScreen.tsx`; Settings Security + `WALLET_SENSITIVE_ACTION_AUTH.airgapKeyshareExport`. Native `BbmtSecureRandom` / `BBMTLibNativeModule.secureRandom`.
