...
> **Patch release:** Explorer traffic is now observable as a slow/fail hint instead of silent hangs; Send no longer treats a timeout as a zero balance. Mempool REST hosts are a checkable list (like Nostr relays) with ranked failover over enabled URLs only; the header shows `host +N` when a pool is active. A keyshare can move airgap via encrypted animated QR (Settings two-step PIN-then-QR export, Welcome restore) locked with a CSPRNG 6-digit PIN. Dimmed modals use a shared frosted glass blur backdrop. Android Play packaging is camera/GPS-optional, LAN pairing uses Nearby Wi-Fi on Android 13+, and sharing a keyshare from WhatsApp/Telegram actually opens Bold. Branta merchant verify is **opt-in (default off)** with SDK **3.2.1** address-mismatch / tamper detection; history shows the merchant name (not “Paying undefined”). MPC on LAN/Nostr queues round payloads so a fast peer cannot poison the current round, and pairing shows a live connection-quality chip. Home-screen camouflage uses bundled launcher identities (QuickCalc, Notes, Weather, Files) without duplicating the Bold tile; settings show the real launcher artwork, and when camouflage is on the lock screen hides Bold branding.

### Added
- **Glass modal backdrops** — dimmed RN Modals share `GlassModalOverlay` (native blur via `@sbaiahmed1/react-native-blur` + lighter `modalBackdropGlass` tint). Centered cards and bottom sheets (Legal, Currency, Entropy, Airgap export) both frost the screen behind a sharp card; tap-outside dismiss stays where it already existed; MPC progress modals stay non-dismissible. Camera/scanner full-screens stay opaque black.
- **UTXO details + tags** — tapping a coin on the UTXOs tab opens a details sheet (amount in BTC/sats, path, explorer). Optional **tag** sits at the top of the sheet (`e.g. savings, cold-storage`): **2–64** ASCII letters/digits/`_`/`-` only; empty clears. Save closes the modal with a toast; autocomplete shows compact chips only while typing (not on empty focus). List chips use navy on light and teal on dark so they stay readable. Tags are stored by outpoint so a UTXO resync cannot wipe them.
- **Send coin control** — when the UTXOs tab is enabled, Available Balance opens a checklist of coins (tag + `abcd~wxyz@vout` / short address). Amount + fee cannot exceed the checked total; only those UTXOs are passed into fee estimate, spending hash, QR, and MPC. Unchecked coins are not spent. Default Send (no checks) is unchanged.
- **Provider health + offline sandbox** — header mempool pill shows a theme-aware health dot (30s tip-hash probe) and toggles an in-app **online/offline** sandbox (default online). Settings → Network Providers has the same switch. Offline uses cached data and blocks Internet and LAN. **Tap to refresh** (Home / UTXOs bar, pull-to-refresh, long-press full sync) while offline opens `WalletOfflineSandboxModal` (Keep offline / Go online / ✕) instead of only a toast; Go online turns the sandbox on then runs the pending refresh.
- **QR scanner elapsed time** — continuous scanners (airgap restore, Send, PSBT) show a live elapsed chip (`12s` / `1:05`) on the HUD and on Android’s native ZXing status line; overlay uses a glass card, corner brackets, and recovered/percent bar. Android ZXing elapsed is wall-clock from scan start (JS timers pause behind the capture activity).
- **Airgap QR keyshare** — Settings → Security (biometric-gated) is a two-step export on the glass modal: CSPRNG **6-digit PIN** first (“Eyes on these two screens only”; grouped `XXX XXX` display), then **Show QR** hides the PIN and plays an animated `ur:bytes` QR of AES-encrypted keyshare ciphertext (`sha256(PIN)` key). **Restart frames** rewinds the fountain encoder; **Speed** cycles Default (500B / 500ms) → Medium (400B / 330ms) → Fast (300B / 250ms) and restarts the QR (seqLen must match on the scanner). Welcome **Restore via Airgap QR** enters that PIN (`inputMode: pin`), then continuous-scans until the UR completes (fountain can finish before N/N) and uses the same import path as a `.share` file (`pendingRestore` indexing). File backup/restore still uses a user-chosen password.
...
- **Send balance recheck** — if the displayed balance looks empty, a live recheck that times out or fails toasts and does **not** map to 0 BTC / false “Insufficient Balance”; send still opens when coins were already on screen.
- **MPC connection quality chip** — LAN and Nostr pairing progress show Best / Medium / Low / Connecting from EWMA RTT and consecutive publish failures (pairing screens only).
...
- **Camouflage PIN** — App Icon PIN toggle (default **off** on Bold). Camouflage tiles force PIN on. With PIN on (Bold or camouflage), lock screen asks for it before the OS fingerprint sheet. **3 wrong PINs** lock the pad for **30s** (fail count + until timestamp in SQLite `app_config`; survives app kill). Forgot PIN: uninstall or delete the wallet, restore the keyshare — no PIN after restore.
- **Branta opt-in** — Wallet settings **Branta** section (Off/On), last under Wallet after Security, Address Type, Network Providers, and Nostr Relays. Merchant verify is **off by default**. When off, Send treats Branta QRs as normal BIP-21, `brantaInitiated` stays false, history/details hide merchant names/checkmarks (stored labels are kept), and verified-tx backfill does not run until the user turns Branta on.
- **Navigation menu style** — Settings → App → **Menu** toggles docked (full-width tab bar) vs floating (**default**: centered bottom pill over full-height screens, max 90% width, horizontal scroll when tabs overflow). Shared 12pt corners on the pill and active item; soft elevation; light theme-aware hairline (`blackOverlay10` / `whiteOverlay12`) on the menu and lock FAB. Preference stored as `nav_menu_style`.
- **MPC keep-alive** — wallet setup and co-signing keep the process alive in the background on Android (foreground service + progress notification) and keep the screen awake on iOS with a short background task plus a complete/fail local notification. Copy no longer says “don’t leave the app”; Android can switch apps (don’t swipe Bold away), iPhone can lock but switching apps may interrupt. Camouflage identities use generic notification text. If Android notifications are denied, the ceremony modal says so and asks the user to stay on screen. iOS posts a one-shot “Return to Bold” local warning when the app backgrounds or the background task expires. Android can offer an optional Ignore Battery Optimizations prompt (skipped when camouflaged). iPhone 16.1+ can show a Lock Screen / Dynamic Island Live Activity for progress while the process is still running (display only; it does not keep MPC alive after iOS suspends the app).

- **Home refresh is one session** — tap, load, and resume pause `SyncCoordinator`, fetch active-set balance + txs (price in parallel), paint SQLite, then resume without kicking a full cycle. Coordinator no longer force-rediscovers HD on every Home start; same network updates config in place. Address HTTP concurrency is `min(3, enabledCount)` when 2+ mempool providers are enabled, otherwise 1.

- **Mempool providers checklist** — Settings → Network Providers mirrors Nostr Relays: curated hosts (GitHub `mempool-space-hosts`) with per-URL enable/disable, add/delete/paste, Defaults, and Save (at least one must stay on). Enabled URLs are the **only** failover / fee (`setFeeAPIs`) / sync pool; disabled hosts are never contacted (request URLs pointing at a disabled host are rewritten onto the enabled set). Failover order prefers recent ok rate + latency (cold hosts mid-rank, known-bad last). Setup **Validate and Proceed** exclusive-enables the typed URL (even a curated public mirror); **Skip** keeps the curated pool all on. Testnet stays locked to `mempool.space/testnet`. Header pill shows `host` or `host +N` from the enabled count (a11y includes backup count). Legacy custom `api_mainnet` migrates to exclusive-enable; stored primary always re-syncs to the first enabled URL.
- **Mempool provider Check** — Settings → Network Providers **Check** runs one tip-hash probe per listed endpoint (`GET …/blocks/tip/hash`) and updates Healthy / Slow / Unreachable on each row. Does not loop; the 30s header poller still only hits the primary. Disabled while the wallet sandbox is offline.

- **Stale fee rates** — if the fee API fails, Send can keep DB-cached rates and show a one-liner instead of blocking.
- **Broadcast Alert copy** — native `PostTx` `failed after N attempts: …` maps to short reasons (already spent, fee too low, API unreachable); HTTP still native, not MempoolClient.
- **Tx details confirmations** — tip height goes through `MempoolClient`; failure still only hides the count.
- **Pending vs confirmed visualizer** — compact track follows the parent details chip (`status.confirmed === true`); a tip-height poll no longer paints “Confirmed in block #…”.
- **Tx details visualizer collapse** — scrolling the history details sheet shrinks the compact track (status line fades, padding tightens, graphic shifts up) so amount / inputs / on-chain rows get more room; scrolling back to the top restores full height. Track (origin, cubes, block numbers) stays on screen.
...
- **Branta SDK 3.2.1** — after ZK decrypt, the pay-to address is checked against the BIP-21 QR address (bech32 case-insensitive); mismatch is treated as tamper and the send stays a normal BIP-21 flow.
- **App icon settings** — compact one-row preset picker (same artwork as the launcher icon) plus a single Unlock PIN row. Stored `alternative` maps to QuickCalc after upgrade. Old Bold tile may linger until the launcher refreshes (Pixel/Samsung cache).
- **Camouflage lock screen** — when a non-Bold launcher identity is selected, LoadingScreen shows that icon and name only: no Bold logo, bitcoin particles, version/update chip, manchette, or update modal. A **4-digit PIN** is required for camouflage (toggle default **off** on Bold; optional extra PIN on Bold if turned on). PIN is entered first, then the system biometric sheet. That OS sheet still shows Bold Wallet on Android/Samsung. Forgot PIN: uninstall or delete the wallet and restore the keyshare — no PIN after restore.
...
- **Prefs live in SQLite** — EncryptedStorage now holds only the `keyshare` blob. Display format, camouflage PIN, app icon choice, dev mode, PSBT first-visit, battery don’t-ask, and `keyshare_meta` all use `app_config` (one-time copy from leftover EncryptedStorage keys). Derived `btcPub` is no longer persisted.
- **Header provider on testnet** — pill no longer overlays the mainnet enabled pool (`host +N`); shows `mempool.space/testnet` only.
- **Network Providers health badge** — Healthy / Slow / Unreachable are filled pills (green / amber / red) with a status dot, not tinted body text. Readable on light and dark cards.
- **UTXO details modal width** — nested `KeyboardAvoidingView` under the glass overlay no longer shrink-wraps the card; overlay centers content in a full-width slot so percentage widths resolve correctly.
...
- **Airgap export Restart frames button** — label is optically centered with the icon; secondary styling uses `theme.colors.secondary` and light/dark label tints instead of hardcoded gray overlay.
- **Airgap restore scanner close** — Android ZXing continuous scan stops before AES import so the camera does not stay up after the keyshare is already written; Welcome shows **Importing keyshare…**.
- **Airgap restore progress** — scanner shows recovered fragments (fountain can finish before N/N); export frame counter wraps circularly (`Frame X of N`).
- **Android airgap restore double scanner** — continuous scan is native ZXing only (the RN HUD no longer sits under the capture activity). Back dismisses the camera and returns to restore.
- **Android ZXing recovered-frame overlay** — elapsed, unique UR `seq/seqLen` counts, and light haptic on new frames run **inside** the capture activity (JS timers and `HapticFeedback` freeze while MainActivity is paused). Haptics follow the in-app toggle via `setScanHud`. `VIBRATE` permission added.
- **iOS scanner HUD parity** — Vision Camera overlay uses the same unique UR seq tracker: live `N / T (P%)` bar from the first multipart frame, elapsed chip, and `HapticFeedback.light()` on new frames (respects the in-app haptics toggle). Decoder progress still replaces the local count when the fountain reports recovered fragments.
- **ZXing elapsed stuck at 0s** — status line elapsed is computed from scan-start `Date.now()` on each frame/progress update, not a JS interval that does not tick while the capture activity is in front.
- **Airgap restore stuck at 99% / N/N** — overlay unique-seq counts ignored fountain mix frames (`seq > seqLen`), so Fast mode could show `N/N` while the decoder was still incomplete. Overlay now counts original fragments only and stays off 100% until the camera closes.
- **Airgap restore stuck behind ZXing after complete** — fountain can finish below N/N (e.g. `334/470`). Import was deferred with `setTimeout(0)`, which does not run while MainActivity is paused behind the capture activity (same as elapsed `0s`). Back flushed the timer and revealed User Preferences with the keyshare already imported. Complete now stops ZXing and imports on the scan event (PSBT already did this); native finish pauses the barcode view, shows **Importing keyshare…**, then `finish()`. Emit still happens outside the native lock.
- **Airgap restore HUD frozen ~60%** — overlay only counts original `seq` 1..seqLen. After the first pass the sender emitted mix-only frames (`seq > seqLen`), so unique originals stopped growing even while mix was still reconstructable. Export now rewinds the fountain encoder every `2×seqLen` frames so missed originals replay; overlay shows a live **Mix N — keep scanning** line; JS dbg logs progress/errors/complete; native emit records last frame only after a successful JS event.

### Technical Details
- **Version**: `package.json` **4.0.6**; Android **`versionCode` 68** / **`versionName` 4.0.6**; iOS build **68** / **`MARKETING_VERSION` 4.0.6**.
- **Glass modals**: `@sbaiahmed1/react-native-blur` **6.0.1**; `components/GlassModalOverlay.tsx`; `theme.colors.modalBackdropGlass`; Jest mocks `BlurView` in `jest.setup.js`; iOS pod `ReactNativeBlur`.
- **Mempool providers**: `services/mempoolProvidersStore.ts` (`CONFIG_KEYS.MEMPOOL_PROVIDERS_ENTRIES`, `exclusiveEnable`, `getHeaderProviderDisplay`, `subscribeMempoolProviders`); `components/MempoolProvidersEditor.tsx`; Settings Network Providers + `UserPreferenceScreen` exclusive-enable / Skip; `MempoolClient` ranked `_getUrlsToTry` (enabled-only rewrite); `mempoolHealth` per-host window + `rankedHosts`; `NetworkContext` / `WalletService` seed `setPublicBases` + `setFeeAPIs` from enabled URLs only.
- **Health / send**: `services/mempoolHealth.ts`, `MempoolClient.ts`, `sendBalanceRecheck.ts`, `broadcastErrorMessages.ts`, `feeUtils.ts`, `CacheIndicator.tsx`, `syncFillWindow.ts`, `SyncCoordinator.ts`, `walletRefreshSession.ts`, `rateLimitRetry.ts` (`mapPool`, `MULTI_PROVIDER_SYNC_CONCURRENCY`). Settings Check uses `probeMempoolApiBasesOnce` (one pass over listed roots); the 30s poller still probes only `apiBase`.
- **Online/offline sandbox**: `services/walletOnlineStore.ts`, `providerHealthPoller.ts`, `components/Header.tsx`, `components/WalletOfflineSandboxModal.tsx`, `screens/WalletHome.tsx`, `screens/UtxosScreen.tsx`, `screens/WalletSettings.tsx`; fetch + native HTTP gated at boot.
- **MPC keep-alive**: `services/mpcKeepAlive.ts`, `MpcKeepAliveService.kt` (`specialUse` FGS, Wifi/Multicast locks), `MpcKeepAliveModule` (Android/iOS idle timer + `beginBackgroundTask` + local notifications + iOS Live Activity); pairing screens start/stop through persist; `POST_NOTIFICATIONS` / `FOREGROUND_SERVICE_SPECIAL_USE` / `REQUEST_IGNORE_BATTERY_OPTIMIZATIONS`. Play Console **Special use** declaration (not in the APK): App content → Foreground service → Special use; subtype already in `PROPERTY_SPECIAL_USE_FGS_SUBTYPE`; duration minutes (preparams) or ~60–120s (keysign/keygen) then `stopForeground`; not `dataSync` / mic / location / camera FGS. Device QA bullets live in `android/scripts/check-play-manifest.sh`.
- **Nav menu style**: `CONFIG_KEYS.NAV_MENU_STYLE` / `services/navMenuStore.ts` (absent = floating); Settings App → Menu; `components/FloatingTabBar.tsx` overlays MainTabs (`tabBar` height 0 / absolute); lock FAB elevation + matching border in `App.tsx`.
...
- **Camouflage**: `IconChangerModule.java`, `MainApplication.kt`, `services/camouflageIcon.ts`, `services/camouflagePresets.ts`, `services/camouflagePin.ts`, `CONFIG_KEYS.CAMOUFLAGE_PIN_FAIL_COUNT` / `CAMOUFLAGE_PIN_LOCKED_UNTIL` / `CAMOUFLAGE_PIN_HASH` / `CAMOUFLAGE_PIN_ENABLED`, `components/CamouflagePinPad.tsx`, `screens/WalletSettings.tsx`, `screens/LoadingScreen.tsx`, `assets/camouflage/*.png`; aliases `.DefaultIconActivity` / `.AlternativeIconActivity` / `.NotesIconActivity` / `.WeatherIconActivity` / `.FilesIconActivity`. Settings selection and lock-screen branding read the enabled launcher alias; wallet wipe resets the alias to Bold.
- **EncryptedStorage**: only `keyshare`. `services/encryptedPrefsMigration.ts` copies leftover prefs into `app_config` (`encrypted_prefs_migration_done`, preserved on wallet delete). `getKeyshareMetadata` / `saveKeyshareMetadata` are SQLite-primary (`keyshare_meta_json`).
- **History**: `MerchantLabelRepository.ts`, `TransactionList.tsx`, `txVisualizerPhase.ts`, `TransactionVisualizer.tsx` (`collapseProgress` SharedValue), `TransactionDetailsModal.tsx` (Reanimated scroll-linked collapse).
- **QR scanners**: `components/QRScannerHud.tsx` (`useUrScanFrameHud`) / `utils/scanElapsed.ts` / `utils/urBytesQr.ts` (`recordUrScanFrame`, `urScanHudFromUniqueSimple`); iOS Vision Camera HUD matches Android ZXing elapsed + unique original-fragment frames + haptic. Android continuous = native ZXing HUD. Patch `rn-barcode-zxing-scan` `setScanHud` / emit outside lock / `finishContinuousScan` (pause + Importing… + finish). Airgap complete stops ZXing and imports on the scan event — no `setTimeout` behind the capture activity.
- **Airgap QR**: `services/airgapPin.ts`, `services/secureRandom.ts`; `utils/urBytesQr.ts` (`bufferToUr` / `receiveUrBytesPartAsBase64`, `UR_AIRGAP_SPEEDS` Default/Medium/Fast, `urSeqLenFromPart`, `urFountainWrapAfter`); `components/AnimatedUrQr.tsx` (`restartNonce`, wrap after `2×seqLen`, `maxFragmentLen`, `frameIntervalMs`); `AirgapKeyshareExportModal.tsx` (PIN → QR steps, speed cycle); `QRScanner.stopAndroidContinuousScan`; `KeyshareImportPasswordModal.tsx` (`inputMode: pin`); Welcome restore in `ShowcaseScreen.tsx`; Settings Security + `WALLET_SENSITIVE_ACTION_AUTH.airgapKeyshareExport`. Native `BbmtSecureRandom` / `BBMTLibNativeModule.secureRandom`.
- **Branta**: `@branta-ops/branta` **3.2.1**; `CONFIG_KEYS.BRANTA_ENABLED` (default off); `isBrantaEnabled()` gates Send, Home, list/details, and verified-tx backfill.
- **Coin control**: `utxo_tags` + `UtxoTagRepository`; `services/utxoCoinControl.ts` (`sanitizeUtxoTagDraft` / `isValidUtxoTag`); `UtxoDetailsModal` (save → toast + close); Send exclusive-pool JSON (no native MPC signature change).
...
### Fixed
- **[#71](https://github.com/BoldBitcoinWallet/BoldWallet/issues/71) — Android SIGABRT in `BarcodeZxingScanModule.onActivityResult`** — patch `rn-barcode-zxing-scan`: clear callback before invoke, ignore unrelated request codes, bail when React instance is inactive, swallow stale-callback `RuntimeException`; add `stopQrReader()`; trim patch-package artifact noise from `android/build/` transforms.
- **Never-overwrite wallet** — wallet cannot be silently replaced by a second keyshare import (setup, restore, or share sheet).
- **iOS build (Xcode 26.4)** — Share Extension bridging header scoped per target (no project-wide `SWIFT_OBJC_BRIDGING_HEADER` bleed); `#import <React/RCTEventEmitter.h>` before `BoldWallet-Swift.h` in **AppDelegate.mm**; `import UIKit` in `KeyshareShareAppDelegate.swift` for launch-option URL handling.

### Technical Details
- **Version**: `package.json` **4.0.1**; Android **`versionCode` 61** / **`versionName` 4.0.1**; iOS build **61** / **`MARKETING_VERSION` 4.0.1**.
- **New modules**: `services/walletGuard.ts`, `services/incomingUrlRouter.ts`, `services/incomingUrlBridge.ts`, `services/incomingShareBridge.ts`, `services/incomingFileClassifier.ts`, `services/keyshareImport.ts`, `services/psbtImport.ts`, `components/IncomingShareHandler.tsx`, `components/IncomingUrlHandler.tsx`, `components/KeyshareImportPasswordModal.tsx`, `ios/BoldWalletShareExtension/*`, `ios/IncomingUrlModule.*`, `ios/KeyshareShareModule.*`, `ios/KeyshareShareAppDelegate.swift`, `android/.../IncomingUrlModule.kt`, `android/.../KeyshareShareModule.kt`.
...
- **Shared wallet-setup orchestration** — `walletSetupOrchestrator` + `lanMpcSetup` / `lanMpcTransport` centralize LAN/Nostr keygen prep, trio role assignment, relay reachability, and keyshare persistence (including **Nostr `nsec` embedded in the DKLS keyshare** for signing without passing secrets through JS).
- **MPC progress UX (DKLS + GG18)** — `mpcProgress` / `mpcProgressUi` map native keygen/keysign steps to percent and phase labels; send flows show UTXO-prep progress; pairing screens surface backend-aware prepare copy (seconds for DKLS `helloDkg` vs minutes for GG18 preparams).
- **Wallet Information (Devices)** — fingerprint color band, duo/trio phone icons, key-index badge, TSS backend logos, and **DKLs23 (libtss)** vs **GG18 (BNB)** from keyshare metadata; **Created At** tap shows full localized setup time.
- **Wallet biometric gate** — `walletBiometricAuth` prompts before dev keyshare inspector, Settings backup/delete, and enabling developer mode (shared with lock-screen biometrics).
...
- **Nostr trio send/sign** — participating npubs CSV is **local + selected peer** only; session id still uses the full committee for coordination.
- **Sync and mempool traffic (from 3.1.1)** — `SyncCoordinator` runs price/balances/UTXOs/transactions **sequentially** after HD discovery; address `/txs` fetches use **429 retry** and per-attempt timeouts.
...
- **BBMTLib NOTICE** — pragmatic third-party attribution for **libtss**, **frost**, **DKLs23**, **go-nostr**, and **tss-lib**, with **BoldBitcoinWallet** org forks as sustained distribution mirrors.
...
- **GG18 Go dependency pin** — upgraded BNB TSS library from `github.com/bnb-chain/tss-lib/v2 v2.0.2` to `github.com/bnb-chain/tss-lib/v3 v3.0.0` (module path migration to `/v3` across `BBMTLib/tss/*`); **`go.mod` replace** resolves **`v3.0.0`** from **`github.com/BoldBitcoinWallet/tss-lib`** (org mirror of bnb-chain).
...
- **MPC security hardening (DKLS23 + GG18)** — sensitive logging redaction in Go (`sanitizeLogMessage`), iOS/Android (no `raw_json` keyshare logs), and native modules; pre-agreement Nostr paths use **subscribe-only** relay reads with strict JSON payload validation (`attempt_id`, nonce format, timestamp freshness); GG18 `UpdateFromBytes` runs on a single goroutine (no concurrent party mutation); inbound parse errors no longer echo raw wire payloads.
- **LAN transport integrity** — relay envelopes use SHA-256 digest (`d1:`) and, when a session key is present, HMAC-SHA256 auth tags (`h1:`); receive path rejects mismatched envelopes.
- **MPC supply-chain resilience (libtss + tss-lib)** — DKLs23 builds clone **`BoldBitcoinWallet/libtss`** (tag **`boldwallet-v4.0.0-libtss-ae1f891`**, commit **`7a712f345b710f98e7f7a26582427ad814852a63`**) with **`vendor/frost`** and **`vendor/DKLs23`** submodules; Rust deps resolve from **`BoldBitcoinWallet/frost`** and **`BoldBitcoinWallet/NDKLs23`** at pinned revs; **`DerivePathWithChainCode`** is baked into the fork (runtime **`derive-path-with-chain-code.patch`** removed); **`setup-libtss.sh`** / CI use **`git clone --recurse-submodules`**.
- **libtss supply-chain pin** — CI and local builds verify **`BoldBitcoinWallet/libtss`** at commit **`7a712f345b710f98e7f7a26582427ad814852a63`** (`.github/actions/setup-libtss/action.yml`, `BBMTLib/mobile-deps/libtss/build-libtss.sh`).
- **DKLS nonce-reuse handling** — signing paths map libtss nonce-reuse failures to `ErrNonceReuseDetected` and fail closed (`BBMTLib/dkls/security.go`).
- **DKLS availability hardening** — bounded missing-fragment retries (`maxMissingFragmentRetries = 6`) to reduce griefing from withheld MPC fragments.
- **Memory hygiene** — sensitive decode buffers cleared after use in keyshare import and GG18 message parsing (`clear()` helpers).
- **libtss init** — `initLibtss()` attempts `OptMlock` with safe fallback when `CAP_IPC_LOCK` is unavailable.
...
- **GG18 dependency baseline**: `github.com/bnb-chain/tss-lib/v3` pinned to **`v3.0.0`**, resolved via **`github.com/BoldBitcoinWallet/tss-lib/v3`** (`BBMTLib/go.mod` replace).
- **DKLs23 dependency baseline**: **`BoldBitcoinWallet/libtss`** @ **`7a712f3…`** with vendored **frost** / **NDKLs23** submodules; org mirrors: **`BoldBitcoinWallet/frost`**, **`BoldBitcoinWallet/NDKLs23`**, **`BoldBitcoinWallet/libtss`**, **`BoldBitcoinWallet/tss-lib`**.
...
- **New / notable modules**: `BBMTLib/dkls/mpc_matrix_test.go`, `BBMTLib/dkls/party_test.go`, `BBMTLib/dkls/security.go`, `BBMTLib/scripts-dkls/dkls-mpc-matrix-test.sh`, `BBMTLib/scripts-dkls/security-release-gate.sh`, `.github/scripts/check-sensitive-logs.sh`, `BBMTLib/tss/nostrtransport/relay_publish.go`, `BBMTLib/dkls/*`, `services/tssBackend.ts`, `services/walletSetupOrchestrator.ts`, `services/lanMpcSetup.ts`, `services/mpcProgress.ts`, `services/sendBtcPrepare.ts`, `services/walletBiometricAuth.ts`, `utils/fingerprintPillColors.ts`, `components/KeyshareJsonTree.tsx`, `components/TransactionFlowDiagram.tsx`, `components/SigningTxRecap.tsx`, `components/PairingSpendStickyFooter.tsx`, `components/transactionFlowUtils.ts`, `hooks/useSendTxPreview.ts`, `types/transactionFlow.ts`, `ios/TssShim.swift`, `ios/BbmtBridge.mm`; tests `__tests__/walletRouteGuard.test.ts`, `__tests__/walletKeyshareRefresh.test.ts`, `__tests__/walletKeysharePresence.test.ts`, `__tests__/fingerprintPillColors.test.ts`, `__tests__/transactionFlowUtils.test.ts`, `__tests__/lanMpcSetup.test.ts` (GG18 + DKLS LAN signing, discovery payload).
